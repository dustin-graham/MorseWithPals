...
class MorseCodeConverter {
    ...
    static String stringPattern(char c) {
        long[] pattern = pattern(c);
        String out = "";
        for (long l : pattern) {
            if (l == DOT) {
                out += ".";
    ...
    static String stringWordPattern(String word) {
        String out = "";
        for (int i = 0; i < word.length(); i++) {
            char c = word.charAt(i);
            String charPattern = stringPattern(c);
            if (i < word.length()-1) {
                charPattern += ",";
            }
            out += charPattern;
    ...
    static String stringPhrasePattern(String phrase) {
        String out = "";
        String[] words = phrase.split(" ");
        for (int i = 0; i < words.length; i++) {
            String wordPattern = stringWordPattern(words[i]);
            if (i < words.length-1) {
                wordPattern += ";";
            }
            out += wordPattern;
